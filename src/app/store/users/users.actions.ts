...
import { UserListFilter, UserListItemModel, UserModel } from '../../core/models/users/users.models';
...
export const loadUsersSuccess = createAction(
  `${key} Load Users Success`,
  props<{ response: PaginatedResponse<UserListItemModel> }>(),
);
export const loadUsersFailure = createAction(`${key} Load Users Failure`);
export const setFilter = createAction(
  `${key} Set Filter`,
  props<{ filter: Partial<UserListFilter> }>(),
);

// POST /account/register
export const registerUser = createAction(
  `${key} Register User`,
  props<{ command: RegisterUserCommand }>(),
);
export const registerUserSuccess = createAction(
  `${key} Register User Success`,
  props<{ item: UserModel }>(),
);
