...
describe('workProjectsReducer', () => {
  it('stores a loaded page and stops loading', () => {
    const loading = workProjectsReducer(
      initialWorkProjectsState,
      WorkProjectsStoreActions.loadProjects({
        filter: {
          page: 1,
          pageSize: 10,
          sortBy: 'createdAt',
    ...
    const loaded = workProjectsReducer(
      loading,
      WorkProjectsStoreActions.loadProjectsSuccess({
        response: {
          items: [],
          totalCount: 4,
          page: 1,
  ...
  it('removes a deleted project from the current page', () => {
    ...
    const result = workProjectsReducer(
      state,
      WorkProjectsStoreActions.deleteProjectSuccess({ id: 'project-id' }),
    );
